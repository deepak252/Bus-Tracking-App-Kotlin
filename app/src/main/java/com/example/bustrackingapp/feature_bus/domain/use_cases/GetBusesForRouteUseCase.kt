...
class GetBusesForRouteUseCase @Inject constructor(
    private val busRepository: BusRepository
) : ApiHandler(){
    operator fun invoke(routeNo : String) : Flow<Resource<List<BusWithRoute>>> = makeRequest(
        apiCall = {
            busRepository.getBusesForRoute(routeNo)
        }
    )
}